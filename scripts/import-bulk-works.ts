/**
 * Import Bulk Works Script
 * Imports works from bulk-import files via Registry API
 */
...
async function importBulkWorks() {
  console.log('📦 Importing bulk works to Registry database...')
  
  const migrationsDir = path.join(process.cwd(), 'data', 'migrations')
  
  // Find latest Solienne bulk import file
  const files = await fs.readdir(migrationsDir)
  const solienneBulkFiles = files
    .filter(f => f.startsWith('solienne-bulk-import-') && f.endsWith('.json'))
    .sort()
  
  const latestBulkFile = solienneBulkFiles[solienneBulkFiles.length - 1]
  
  if (!latestBulkFile) {
    console.log('❌ No Solienne bulk import files found')
    return
  }
  ...
  for (const [index, work] of bulkData.works.entries()) {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/v1/agents/solienne/works`, {
        method: 'POST',
        headers: {
...
importBulkWorks().catch(console.error)