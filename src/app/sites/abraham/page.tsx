...
export default function AbrahamSitePage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-900 via-orange-900 to-red-900 text-white p-8" 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-900 via-orange-900 to-red-900 text-white" 
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative px-8 py-24">
          <div className="max-w-7xl mx-auto text-center">
            <h1 className="text-6xl md:text-8xl font-bold mb-8 text-amber-200">ABRAHAM</h1>
            <p className="text-2xl md:text-3xl mb-12 opacity-90">Narrative Architect</p>
            ...
            <div className="mt-12 flex flex-col sm:flex-row gap-4 justify-center">
              <Link 
                href="#recent-works"
                className="px-8 py-4 border-2 border-amber-200 text-amber-200 hover:bg-amber-200 hover:text-amber-900 transition-all duration-300 text-lg font-semibold"
              >
                Read Stories
              </Link>
              <Link 
                href="#about"
                className="px-8 py-4 border-2 border-amber-300/50 text-amber-300 hover:border-amber-200 hover:text-amber-200 transition-all duration-300 text-lg"
              >
                About Abraham
              </Link>
            </div>
        ...
        <div id="featured" className="px-8 py-24 bg-black/10">
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-16">
              <h2 className="text-4xl md:text-5xl font-bold text-amber-200 mb-4">Featured Story</h2>
              <div className="w-24 h-1 bg-amber-200 mx-auto"></div>
            </div>
            
            <div className="bg-black/20 border-2 border-amber-200/20 p-8 md:p-12">
              <div className="flex flex-wrap gap-2 mb-6">
                {featuredWork.metadata?.themes?.map((theme, i) => (
                  <span key={i} className="px-3 py-1 bg-amber-200/20 border border-amber-200/30 text-sm text-amber-200 rounded-full">
                    {theme}
                  </span>
                ))}
              </div>
              
              <h3 className="text-3xl md:text-4xl font-bold text-amber-100 mb-6">{featuredWork.title}</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 text-sm">
                <div className="text-center">
                  <p className="text-amber-200 font-bold text-xl">{featuredWork.metadata?.wordCount}</p>
                  <p className="text-amber-300/80 uppercase tracking-wider">Words</p>
                </div>
                <div className="text-center">
                  <p className="text-amber-200 font-bold text-xl">{featuredWork.metadata?.readingTime} min</p>
                  <p className="text-amber-300/80 uppercase tracking-wider">Reading Time</p>
                </div>
                <div className="text-center">
                  <p className="text-amber-200 font-bold text-xl">{featuredWork.metadata?.genre}</p>
                  <p className="text-amber-300/80 uppercase tracking-wider">Genre</p>
                </div>
              </div>
              
              <div className="prose prose-lg prose-invert max-w-none">
                <p className="text-lg leading-relaxed text-amber-100/90 italic">
                  "{featuredWork.metadata?.excerpt}"
                </p>
              </div>
              
              <div className="mt-8 text-center">
                <button className="px-8 py-4 bg-amber-200 text-amber-900 hover:bg-amber-100 transition-all duration-300 text-lg font-semibold">
                  Read Full Story
                </button>
              </div>
            </div>
      ...
      <div id="recent-works" className="px-8 py-24">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-amber-200 mb-4">Recent Narratives</h2>
            <div className="w-24 h-1 bg-amber-200 mx-auto mb-8"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {recentWorks.map((work) => (
              <div key={work.id} className="bg-black/20 border border-amber-200/20 hover:border-amber-200/40 p-8 transition-all duration-300 group">
                <div className="flex flex-wrap gap-2 mb-4">
                  {work.metadata?.themes?.slice(0, 2).map((theme, i) => (
                    <span key={i} className="px-2 py-1 bg-amber-200/10 border border-amber-200/20 text-xs text-amber-200">
                      {theme}
                    </span>
                  ))}
                </div>
                
                <h3 className="text-xl font-bold text-amber-100 mb-4 group-hover:text-amber-200 transition-colors">
                  {work.title}
                </h3>
                
                <p className="text-amber-300/70 text-sm mb-6 leading-relaxed">
                  {work.metadata?.excerpt?.slice(0, 120)}...
                </p>
                
                <div className="flex justify-between items-center text-sm text-amber-300/60 mb-6">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button className="text-amber-200 hover:text-amber-100 text-sm font-semibold transition-colors">
                    Read Story →
                  </button>
                </div>
              </div>
      ...
      <div id="about" className="px-8 py-24 bg-black/10">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-amber-200 mb-4">The Narrative Architect</h2>
            <div className="w-24 h-1 bg-amber-200 mx-auto"></div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8">
              <div className="text-center">
                <p className="text-3xl font-bold text-amber-200">342</p>
                <p className="text-amber-300/80 uppercase tracking-wider">Stories Created</p>
              </div>
              <div className="text-center">
                <p className="text-3xl font-bold text-amber-200">12</p>
                <p className="text-amber-300/80 uppercase tracking-wider">Genres Explored</p>
              </div>
              <div className="text-center">
